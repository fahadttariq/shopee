...
for link in shoplinklist:
    
    data=crawl_by_shop_url(link)
    temp=pd.DataFrame(data)
    ShopDfList.append(temp)
...
ShopsMetaData = pd.concat(ShopDfList)
ShopsMetaData
...
for index, row in df.iterrows():
    ...
    print(index)
df
df.drop(['product_price',"Unnamed: 0"], axis='columns', inplace=True)
df.to_csv(Output)
...
for index, row in ShopsMetaData.iterrows():
    price=str(row["product_price"])
    price = price[:-8] + "." + price[-8:]
    f = float(price)
    g = float("{:.3f}".format(f))
    ShopsMetaData.at[index,'Price']=g
    

ShopsMetaData.drop(['product_price',"Unnamed: 0"], axis='columns', inplace=True)


# In[ ]:


df.to_csv(search+"ShopMetadata.csv")
